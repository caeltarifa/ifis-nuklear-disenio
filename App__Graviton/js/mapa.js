    ...
    'features': [
      {
        ...
        'properties': {
          ...
          'iconSize': [16, 16],
          'description': posiciones[i].nombre
        },
        'geometry': {
          'type': 'Point',
...
lista_lugares.forEach(function (geojson) {
  ...
  el.addEventListener('mouseenter', function(){
    popup.setLngLat(geojson.features[0].geometry.coordinates).setHTML(geojson.features[0].properties.description).addTo(beforeMap);
  });
  ...
  /* el.addEventListener('mouseover',function(){
    alert("hola mundo");
    el.bindPopup(geojson.features[0].properties.message).closePopup();
  }) */
...
var lista_lugares2 = []
for (var j = 0; j < posiciones2.length; j++) {
  var geojson2 = {
    'type': 'FeatureCollection',
    'features': [
      {
        'type': 'Feature',
        'properties': {
          'message': posiciones2[j].nombre,
          'iconSize': [16, 16],
          'description': posiciones2[j].nombre
        },
        'geometry': {
          'type': 'Point',
          'coordinates': [posiciones2[j].longitude, posiciones2[j].latitude]
        }
      }
    ]
  };
  lista_lugares2.push(geojson2)
}

lista_lugares2.forEach(function (geojson2) {
  var el = document.createElement('div');
  ...
  el.addEventListener('click', function () {
    //window.alert(geojson.features[0].properties.message);
    app.titulo_lugar(geojson2.features[0].properties.message)
  });

  el.addEventListener('mouseenter', function(){
    popup.setLngLat(geojson2.features[0].geometry.coordinates).setHTML(geojson2.features[0].properties.description).addTo(afterMap);
  });
  ...
  // add marker to map
  new mapboxgl.Marker(el).setLngLat(geojson2.features[0].geometry.coordinates).addTo(afterMap);
});
